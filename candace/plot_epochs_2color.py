...
# Plot both channels, plot mean and scale
# Plot one sample at a time, one sample per pdf
...
for s in range(nsamp):
    ...
    plt.gcf().text(0.05, 0.5, "Mean", fontsize=24)
    plt.gcf().text(0.05, 0.2, "Scale", fontsize=24)
    ...
    plt.gcf().text(0.45,0.9, keepers[0]+', ' + keepers[1], fontsize=30)

    h = 3 #1st row: input/ground truth, 2nd row: mean, 3rd row: scale
    w = len(names)
    ...
    for i in range(len(names)):
        ...
        plt.title(epoch, fontsize=24)
        img = _P_mean
        img = np.squeeze(np.stack(map(to_color,img)))
        plt.imshow(img)
        plt.axis("off")

        plt.subplot(h, w, 2*w + i + 1)
        img = _P_scale
        img = np.squeeze(np.stack(map(to_color,img)))
        plt.imshow(img)
        plt.axis("off")

    fig.savefig('/home/ubuntu/candace/plots/ki67_cd45_epochs/' + ch[0] + '_' + ch[1] + '_meanscale_' + str(s) + '.pdf')
